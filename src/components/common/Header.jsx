import React, { useState } from "react";
import { NavLink, Link, useNavigate } from "react-router-dom"; // Import NavLink
import { useDispatch, useSelector } from "react-redux";
import { logout, useCurrentToken } from "../../redux/auth/authSlice";
import { toast } from "sonner";
import Button from "../../utils/Button";
import { FiMenu } from "react-icons/fi";
import img from "../../../public/charity-logo.png";
import { toggleTheme } from "../../redux/toggle/toggleSlice";

const Header = () => {
  const navigate = useNavigate();
  const token = useSelector(useCurrentToken);
  const dispatch = useDispatch();
  const theme = useSelector((state) => state.toggle.theme);
  const dispatchs = useDispatch();

  const handleDarkMode = () => {
    dispatchs(toggleTheme());
  };

  React.useEffect(() => {
    localStorage.setItem("theme", theme);
    document.querySelector("html").setAttribute("data-theme", theme);
  }, [theme]);

  const handleLogOut = () => {
    dispatch(logout());
    toast.success("Log Out Successful");
    navigate("/");
  };

  let Links = [
    { name: "Home", link: "/" },
    { name: "Donations", link: "/donations" },
    { name: "Leaderboard", link: "/leaderboard" },
    { name: "Community", link: "/community" },
    { name: "About Us", link: "/volunteer" },
    ...(token ? [{ name: "Dashboard", link: "/dashboard" }] : []),

    // { name: "ABOUT", link: "/" },
    // { name: "BLOG'S", link: "/" },
    // { name: "CONTACT", link: "/" },
  ];
  let [open, setOpen] = useState(false);

  return (
    <div className="shadow-md w-full left-0 top-0 z-20 sticky">
      <div
        className={` md:flex black ${theme} bg-white items-center justify-between  py-4 md:px-10 px-7`}
      >
        <div className="font-bold text-2xl cursor-pointer flex items-center font-[Poppins] text-gray-800">
          <span className="text-3xl text-indigo-600 mr-1 pt-2">
            <FiMenu className="hidden" name="logo-ionic"></FiMenu>
          </span>
          <img className="w-30" src={img} alt="" />
        </div>

        <div
          onClick={() => setOpen(!open)}
          className="text-3xl absolute right-8 top-6 cursor-pointer md:hidden"
        >
          <FiMenu name={open ? "close" : "menu"}></FiMenu>
        </div>

        <ul
          className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static mode ${theme} md:z-auto z-[1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${
            open ? "top-15 " : "top-[-490px]"
          }`}
        >
          {Links.map((link) => (
            <li key={link.name} className="md:ml-8 text-xl md:my-0 my-7">
              <NavLink
                to={link.link}
                className={`white ${theme} hover:text-gray-400 duration-500`}
                activeClassName="text-indigo-600"
                activeStyle={{
                  color: "red",
                }}
                exact
              >
                {link.name}
              </NavLink>
            </li>
          ))}
          {token ? (
            <Link onClick={handleLogOut} to="/register">
              {" "}
              <Button>logOut</Button>
            </Link>
          ) : (
            <Link to="/register">
              {" "}
              <Button>Sign Up</Button>
            </Link>
          )}
          <label className="swap swap-rotate ml-3">
            <input
              type="checkbox"
              onChange={handleDarkMode}
              checked={theme === "dark"}
            />

            <svg
              className="swap-on fill-current w-10 h-10"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            >
              <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
            </svg>

            <svg
              className="swap-off fill-current w-10 h-10"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            >
              <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
            </svg>
          </label>
        </ul>
      </div>
    </div>
  );
};

export default Header;
